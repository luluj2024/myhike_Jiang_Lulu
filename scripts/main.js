...
function doAll() {
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            ...
            const d = new Date();
            //  getDay() -> get the day of the week as an integer value between 0 (Sunday) and 6 (Saturday).
...
function readQuote(day) {
    db.collection("quotes").doc(day).onSnapshot(dayDoc => {                                                     //name of the collection and documents should matach excatly with what you have in Firestore
    ...
    db.collection(collection)
        ...
        .then(allHikes=> {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                ...
                newcard.querySelector('i').id = 'save-' + docID;   //guaranteed to be unique, set ID
...
//         // 向数组字段添加新元素。如果该元素已存在于数组中，则不会重复添加
